...
The tree is a prepared export for release version `0.1.14`, generated from
the exact reviewed SDK build:

- the closed export contains exactly 72 JavaScript and declaration files,
...
- the version advanced to `0.1.14`; immutable `0.1.0` through `0.1.3`, `0.1.5`,
  `0.1.6`, `0.1.7`, `0.1.8`, `0.1.9`, `0.1.10`, `0.1.11`, `0.1.12` and `0.1.13` are never republished or altered, and `0.1.4` remains unused
...
- `scripts/check-packed-files.mjs` — the packed file set must equal exactly
  ...
  bind version `0.1.14` and `millwork` to the exported `dist/cli.js`.
- `scripts/smoke-installed.mjs` — packs the tree, installs the tarball into
  ...
  the installed `millwork` executable, verifies the exact `0.1.14` package and
