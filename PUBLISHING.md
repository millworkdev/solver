...
The tree is a prepared export for candidate version `0.1.6`, generated from
...
- the version advanced to `0.1.6`; immutable `0.1.0` through `0.1.3` and
  `0.1.5` are never republished or altered, and `0.1.4` remains unused because
  it was reserved by the release drill plan.
...
- `scripts/check-packed-files.mjs` — the packed file set must equal exactly
  ...
  bind version `0.1.6` and `millwork` to the exported `dist/cli.js`.
- `scripts/smoke-installed.mjs` — packs the tree, installs the tarball into
  ...
  the installed `millwork` executable, verifies the exact `0.1.6` package and
